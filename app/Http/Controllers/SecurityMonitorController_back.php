...
class SecurityMonitorController_back extends Controller
{
    /**
            ...
            if ($response->successful()) {
                ...
                foreach ($versionRows as $row) {
                    ...
                    if ($cells->length >= 3) {
                        ...
                        $version = trim($cells->item(1)->textContent);

                        if (strpos($product, 'Adobe Commerce') !== false || strpos($product, 'Magento') !== false) {
                            $versions = preg_split('/\s+and\s+|\s*,\s*/', $version);
                            foreach ($versions as $v) {
                                if (!empty($v)) {
                                    $affectedVersions[] = trim($v);
                                }
                            }
                        }
                    }
                ...
                $securityPatch = SecurityPatch::create([
                    'magento_version' => implode(', ', $affectedVersions),
    ...
    private function checkStoresForPatch(SecurityPatch $patch)
    {
        ...
        foreach ($stores as $store) {
            ...
            // Check if store version is affected
            $isAffected = false;
            $affectedVersions = explode(', ', $patch->magento_version);

            foreach ($affectedVersions as $affectedVersion) {
                if (strpos($affectedVersion, $store->magento_version) !== false) {
                    $isAffected = true;
                    break;
                }
            }
